...
class GRUGenomeConfig(object):
    ...
    def __init__(self, params):
        ...
        self._params = [ConfigParameter('num_inputs', int),
                        ...
                        ConfigParameter('gru_node_chance', float, 0.25),]

        # Gather configuration data from the gene classes.
        self.node_gene_type = params['node_gene_type']
        for t in self.node_gene_type:
            self._params += t.get_config_params()
        ...
        for ngt in self.node_gene_type:
            ngt.validate_attributes(self)
...
class GRUGenome(object):
    ...
    def parse_config(cls, param_dict):
        param_dict['node_gene_type'] = [DefaultNodeGene, GRUNodeGene]
        param_dict['connection_gene_type'] = DefaultConnectionGene
        return GRUGenomeConfig(param_dict)
    ...
    def create_node(config, node_id):
        node = None
        if random() < config.gru_node_chance:
            # Create a GRU node gene.
