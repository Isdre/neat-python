...
class GRURecurrentNetwork(object):
    ...
    def activate(self, inputs):
        ...
        for node, activation, aggregation, bias, response, links, calc in self.node_evals:
    ...
    def create(genome, config):
        ...
        for node_key, inputs in node_inputs.items():
            ...
            calc = lambda x: x
            if isinstance(node, GRUNodeGene):
                calc = node.gru_cell.calc
            node_evals.append((node_key, activation_function, aggregation_function, node.bias, node.response, inputs, calc))

        return GRURecurrentNetwork(genome_config.input_keys, genome_config.output_keys, node_evals)
